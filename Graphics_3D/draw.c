...
void generate_sphere( struct matrix * points, 
  ...
  double p, q, x, y, z;
  for (p = 0; p < 1.0001; p += step){
    for (q = 0; q < 1.0001; q += step){
      x = r*cos(M_PI*q) + cx;
      y = r*sin(M_PI*q)*cos(2*M_PI*p) + cy;
      z = r*sin(M_PI*q)*sin(2*M_PI*p) + cz;
...
void generate_torus( struct matrix * points, 
  ...
  for (p = 0; p < 1.001; p += step){
    for (q = 0; q < 1.001; q += step){
      x = r1*cos(2*M_PI*q) + cx;
      y = cos(2*M_PI*p)*(r1*sin(2*M_PI*q) + r2) + cy;
      z = sin(2*M_PI*p)*(r1*sin(2*M_PI*q) + r2) + cz;
      add_point(points, x, y, z);
      add_point(points, x, y, z);
    }
  }
