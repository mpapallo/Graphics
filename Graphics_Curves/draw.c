...
void add_circle( struct matrix * points, 
  ...
  double t = 0;
  double x1, y1;
  double x0 = cx + r * cos(t * 2 * M_PI);
  double y0 = cy + r * sin(t * 2 * M_PI);
...
void add_curve( struct matrix *points, 
  ...
  if (type == HERMITE_MODE){
    cx = generate_curve_coefs(x0, x2, x1-x0, x3-x2, type);
    cy = generate_curve_coefs(y0, y2, y1-y0, y3-y2, type);
  }else{
  ...
  for (t = step; t < 1.000001; t += step){
    ...
    px1 = cubic(cx->m[0][0], cx->m[1][0], cx->m[2][0], cx->m[3][0], t);
    py1 = cubic(cy->m[0][0], cy->m[1][0], cy->m[2][0], cy->m[3][0], t);
