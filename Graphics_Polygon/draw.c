...
void add_sphere( struct matrix * points, 
  ...
  for( lat = latStart; lat < latStop; lat++ ) {
    for ( longt = longStart; 2*longt < longStop; longt++ ) {
      ...
      if( lat == latStop-1 ){ //last slice
	
	int windex = index % num_steps; //connect to first slice

	add_polygon( points,
		     temp->m[0][index], temp->m[1][index], temp->m[2][index],
		     temp->m[0][windex+1], temp->m[1][windex+1], temp->m[2][windex+1],
		     temp->m[0][windex], temp->m[1][windex], temp->m[2][windex]);
	add_polygon( points,
		     temp->m[0][index], temp->m[1][index], temp->m[2][index],
...
void add_torus( struct matrix * points, 
  ...
  for ( lat = latStart; lat < latStop; lat++ )
	...
	int index2 = index % num_steps;

      	if(longt == longtStop - 1){ //last point
