...
void first_pass() {
  ...
  if (!called_basename){
    strcpy(name,"img");
    printf("No basename provided, default set to '%s'\n", name);
  }
  if (!called_frames){
    num_frames = 1;
  }
...
void my_main( int polygons ) {
  ...
  lights[Ka].c[ired] = 200;
  lights[Ka].c[igreen] = 200;
  lights[Ka].c[iblue] = 200;
  
  //We will also store our view vector in here - DON'T CHANGE
  lights[0].l[0]=0;
  lights[0].l[1]=0;
  lights[0].l[2]=1;
  
  //Define a default point light source (this is random)
  lights[1].c[ired] = 300;
  lights[1].c[igreen] = 100;
  lights[1].c[iblue] = 100;
  lights[1].l[0] = 0;
  lights[1].l[1] = 1;
  lights[1].l[2] = 1;
  ...
  for (f=0;f<num_frames;f++){
	...
	draw_polygons( tmp, zbuf, t, g, dcolor, lights, num_lights );
	tmp->lastcol = 0;
	break;
